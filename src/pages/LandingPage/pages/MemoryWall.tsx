...
const MemoryWall = () => {
  ...
  const fetchTributes = async () => {
    try {
      ...
      const result = await dispatch(FetchTribute());
    ...
    if (formData.tribute.trim() && formData.fullName.trim()) {
      try {
        ...
        if (meta.requestStatus === "rejected") {
          ...
          setError(
            err.errorMsg || "Failed to submit tribute. Please try again.",
          );
          return;
        }
        ...
        if (meta.requestStatus === "fulfilled") {
          ...
          const newMemory = {
            id: response.id,
            dateCreated: new Date(response.dateCreated).toLocaleDateString(
              "en-US",
              {
                year: "numeric",
                month: "long",
    ...
    if (formData.tribute.trim() && formData.fullName.trim() && editingMemory) {
      try {
        ...
        if (meta.requestStatus === "rejected") {
          let err = payload as ErrorResponse;
          setError(
            err.errorMsg || "Failed to update tribute. Please try again.",
          );
          return;
        }
    ...
    <section id="leave-a-tribute" className="py-16 px-4 bg-white">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p
            className="text-gray-700 italic max-w-2xl mx-auto mb-8 animate-fadeIn"
            style={{ animationDelay: "0.3s" }}
          >
            "To live in the hearts we leave behind is not to die."
            <br />
            <span className="text-sm">
              Please share your Photos and Memories about David Dari Dwam.
            </span>
          </p>
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <Edit2 className="w-4 h-4 text-gray-400 hover:text-[#deac6c] transition-all duration-300" />
                      </button>
                      <button
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 animate-fadeIn">
                  <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-modalSlideUp">
                    ...
                    >
                      ...
                      <div
                        className="flex gap-4 pt-4 animate-slideRight"
                        style={{ animationDelay: "0.3s" }}
                      >
                        <button
                          type="button"
            ...
            {memories.length > INITIAL_DISPLAY_COUNT && (
              <div className="text-center animate-fadeIn">
                ...
                >
                  {showAll
                    ? "← Show Less"
                    : `See More Tributes (${memories.length - INITIAL_DISPLAY_COUNT} more) →`}
                </button>
              </div>
            )}
